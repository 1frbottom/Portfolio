...
<ul>
<details>
<summary> [ <sub>해결</sub> ] 애니메이션 도입 후 재발한 틱 부하와 이동값 추적 문제 ( Click )</summary>

<br>

>**문제 상황 :**
>
>몬스터 로코모션 애니메이션을 붙이자 프레임이 다시 하락. 또한 풀에서 꺼낸 몬스터가 스폰 지점으로 순간이동하는 프레임이 `달리기` 로 오인되어 애니메이션이 튀는 현상 발생.<br><br>
>
>**원인 분석 :**
>
>- 몬스터 이동을 복제하지 않는 구조 탓에 `AnimInstance` 가 참조할 네트워크 속도값이 없었고, 매 프레임 위치 델타를 직접 계산해야 했음. 풀 스폰 시의 순간이동이 거대한 델타로 잡혀 임계값으로 걸러내는 땜질이 필요.<br><br>
>- 타겟이 없는 유휴 몬스터와 풀 대기 몬스터까지 액터 틱이 켜진 채 유지되고, 화면 밖 몬스터도 포즈를 갱신.<br><br>
>
>**해결 과정 :**
>
>- 이동 컴포넌트가 실제 스윕된 거리를 `Velocity` 로 발행하도록 변경. `APawn::GetVelocity()` 만 읽으면 되므로 `AnimInstance` 에서 위치 추적과 임계값 필터 두 가지를 통째로 제거. 벽에 밀착한 몬스터가 제자리걸음 대신 정지로 표현되는 부수 효과도 확보.<br><br>
>- `ShouldTick()` / `RefreshTickState()` 로 틱 제어를 단일 지점에 일원화. 풀 대기·사망 상태는 틱을 돌리지 않고, 유휴 몬스터는 넉백 슬라이드가 남아 있는 동안만 깨어나도록 처리.<br><br>
>- `OnlyTickPoseWhenRendered` 로 화면 밖 포즈 갱신 제거.<br><br>
>
>**결과 :**
>
>상태 변화 경로마다 틱을 직접 켜고 끄던 코드가 사라져, 유휴 몬스터가 프레임을 태우거나 추격 중 몬스터가 정지하는 두 방향의 버그 가능성을 함께 제거.<br><br>
</details>
</ul>
