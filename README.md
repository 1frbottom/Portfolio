...
>
>**현재 상태 ( 2026.08 / Steam OSS, 리슨 서버 + 클라이언트, 기기 2대, 클라이언트 기준 ) :**
>
>- 처리 부하 : `50`마리 `Frame 6.59ms` → `1,000`마리 `Frame 22.37ms`. 동일 구간에서 `GPU` 는 `4.84ms` → `6.24ms` 로 거의 변동이 없고 `Game 22.30ms` 가 `Frame` 과 일치, 병목이 렌더가 아닌 게임 스레드임을 확인. 개체 수 `20`배 증가에도 `45 FPS` 유지.<br><br>
>- 네트워크 : 컬링거리 내부 클라 복제 액터 `802` 기준 송신 `3.5KB/s` 로, `50`마리 시점의 `5.3KB/s` 와 동일 수준 유지. 초기 스터터링 해소.<br><br>
</details>
</ul>
